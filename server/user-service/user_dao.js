...
const userSchema = require('./userSchema_model').mongoUserSchema;
...
async function validateLoginUser(loginInfo, response) {
    await UserModel.findOne({ username: loginInfo.username }, (err, result) => {
        if (err || !result) {
            log.error(`err in find by user ${loginInfo.username}: ` + err);
        ...
        if (result && bcrypt.compareSync(loginInfo.password, result.password)) {
            ...
            return response.header('auth-token', jwtToken).send({
                username: loginInfo.username,
                messageCode: 'USRV',
...
async function resgisterNewUser(userObj, response) {
    ...
    await newUser.save((err, result) => {
        ...
        return response.send({
            messageCode: '200',
            message: 'Succssfully connectedd',
            username: result.username
        });
    });
}
